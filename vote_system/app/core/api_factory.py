...
class ApiFactory:
    def __init__(
        self, user: User, data_access_layer: AbstractDataAccessLayer, logger
    ) -> None:
        self._user = user
    ...
    def create_voter_api(self) -> VoterApi:
        ...
        return VoterApi(
            user_id=self._user.id, data_access_layer=self._dal, logger=self._log
        )

    def create_admin_api(self) -> AdminApi:
        # TODO validate token for ADMINS
        return AdminApi(
            user_id=self._user, data_access_layer=self._dal, logger=self._log
        )

    def create_user_api(self) -> UserApi:
        # TODO validate token for REGISTERED USERS
        return UserApi(user_id=self._user, data_access_layer=self._dal, logger=self._log)
