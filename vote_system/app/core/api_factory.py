...
class ApiFactory:
    ...
    def create_voter_api(self) -> VoterApi:
        ...
        return VoterApi(token=self._token, data_access_layer=self._dal)
    
    def create_admin_api(self) -> AdminApi:
        #TODO validate token for ADMINS
        return AdminApi(token=self._token, data_access_layer=self._dal)
    
    def create_user_api(self) -> UserApi:
        #TODO validate token for REGISTERED USERS
        return UserApi(token=self._token, data_access_layer=self._dal)
